...
import { chinese, english, getCopy, languageFromPath, languagePath } from '../lib/copy.ts';

function entries(value, path = '') {
  return Object.entries(value).flatMap(([key, child]) => typeof child === 'string' ? [[`${path}.${key}`, child]] : entries(child, `${path}.${key}`));
}
test('English covers every Chinese copy field with no untranslated Chinese', () => {
  const zh = entries(chinese), en = entries(english);
  assert.deepEqual(en.map(([key]) => key), zh.map(([key]) => key));
  for (const [key, value] of en) {
    assert.ok(value.trim().length > 0 || key === '.observation.titlePrefix', key);
...
test('language paths are explicit and do not leak to similar route names', () => {
  ...
  for (const language of ['zh', 'en']) assert.equal(languageFromPath(languagePath(language)), language);
  assert.equal(getCopy('en'), english);
  assert.equal(getCopy('zh'), chinese);
});
test('English retains all focus areas, process steps, limits and projection labels', () => {
  assert.equal(english.interests.length, 4);
  assert.equal(english.steps.length, 4);
  assert.equal(english.boundaries.length, 3);
  assert.deepEqual(Object.keys(english.observation.views), ['oblique','front','side']);
